...
from mpscli.model.builder.utils.DiskModelLoader import parse_fpr, parse_mps
...
class SSolutionBuilder:
    ...
    # callers that need caching shouldd construct an MpbBatchParser with the desired settings and pass it to build_all().

    def build_solution(self, path_to_msd_file: Path) -> Optional[SSolution]:
        models_dir = path_to_msd_file.parent / "models"
        if not models_dir.exists():
            _log.warning(
                "'models' directory not found - no model loaded from: %s",
                path_to_msd_file.parent,
            )
            return None
