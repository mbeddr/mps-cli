...
# directory (the table of contents at the end of the ZIP file) plus small .msd and .mpl XML members and 
# no model bytes are read here.
#
# Produces JarScanResult which SSolutionsRepositoryBuilder uses to drive phases 2-4 and the mpb 
# member list feeds MpbBatchParser, solution_infos feeds JarModelLoader, and jar_stats feeds ParseCache for 
# staleness checks.
...
class JarSolutionInfo:
    ...
    def __init__(self, solution, jar_path_str, mpb_members, mps_members, fpr_prefixes):
        ...
        # [(jar_path_str, member_name)] fed directly to MpbBatchParser.parse_from_jars
        self.mpb_members = mpb_members
        # [member_name] for .mps files within this jar
        self.mps_members = mps_members
        # [prefix] for FPR directories within this jar for e.x. 'module/models/MyModel/'
...
class JarScanResult:
    ...
    def __init__(self):
        # one entry per (.msd, jar) pair and these may have duplicates across jar files..
        ...
        # jar_path_str: (mtime, size) collected during scan and ParseCache.load_sync() uses these to validate 
        # cache staleness without extra calls and each JAR is already open so stat() is cheap here I guess..
